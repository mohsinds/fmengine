...
from fmtrader.agents.runner import CampaignStore, _stub_hypothesize_payload, finalize_campaign, run_generation
...
def _router_for(state: CampaignState) -> LLMRouter:
    ...
    # Empty JSON → hypothesize falls back to multi-strategy grid sample
...
async def finalize_campaign_activity(state_dict: dict[str, Any]) -> dict[str, Any]:
    state = CampaignState.from_dict(state_dict)
    state = finalize_campaign(state, journal=ResearchJournal(), store=CampaignStore())
    return state.to_dict()
...
def temporal_activity_defs() -> list[Any]:
    ...
    try:
        ...
        return [run_generation_activity, checkpoint_activity, finalize_campaign_activity]
    return [
